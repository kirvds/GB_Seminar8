...
def copy_row(filename):
    row_number = int(input("Введите номер строки: "))
    res = read_file(filename)
    if row_number > len(res) or row_number < 1:
        print("Нет такой строки")
        return
    filename_to = input("Введите имя файла для записи: ")
    if not exists(filename_to):
        create_file(filename_to)
    res1 = []
    res1.append(res[row_number-1])
    standard_write(filename_to, res1)
